...
def register_run(config: dict, run_id: str, run_dir: str, pid: int) -> None:
    """Register a new run in the registry.jsonl file."""
...
def update_run_status(config: dict, run_id: str, status: str) -> None:
    """Update the status of a run in the registry.jsonl file."""
    path = _registry_path(config)
    if not path.exists():
        return

    # Read registry.jsonl, split into lines
    lines = path.read_text(encoding="utf-8").splitlines()
    updated = []
    # Each line represents a run
    for line in lines:
        if not line.strip():
            continue
        try:
            # Parse json line into python object
