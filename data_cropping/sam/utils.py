...
def get_latest_result_dir(base_dir: Path) -> Path:
	...
	candidates = [p for p in base_dir.glob("*_result_det_*") if p.is_dir() and p.name.split("_result_det_")[-1].isdigit()]
	if not candidates:
...
def select_or_latest(base_dir: Path, title: str) -> Path:
	...
	root = tk.Tk()
	root.withdraw()
	chosen_dir = filedialog.askdirectory(initialdir=base_dir, title=title) or None
	root.update()
	root.destroy()
