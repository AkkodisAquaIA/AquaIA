...
def find_latest_run_dir(runs_root):
    ...
    valid_run_dirs = [path for path in run_dirs if (path / "best_model.pt").exists()]
    if not valid_run_dirs:
        raise FileNotFoundError(f"No completed run directories with best_model.pt found under {runs_root}")
