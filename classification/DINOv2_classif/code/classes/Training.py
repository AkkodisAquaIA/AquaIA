...
class Training:
	...
	def __init__(self, **kwargs):
		...
		**kwargs: Keyword arguments containing configuration parameters such as:
		...
		"""
		self.optimizer_mapping = {"Adagrad": Adagrad, "Adam": Adam, "AdamW": AdamW, "NAdam": NAdam, "RMSprop": RMSprop, "RAdam": RAdam, "SGD": SGD}
		...
		self.beta_values = [self.param_converter._convert_param(self.loss_params.get("beta_0", 0.0)), self.param_converter._convert_param(self.loss_params.get("beta_1", 0.9999))]
	...
	def create_metric(self, binary_metric, multiclass_metric):
		...
		if name == "MulticlassConfusionMatrix":
			return multiclass_metric(num_classes=self.num_classes, ignore_index=self.ignore_index).to(self.device)
	...
	def initialize_loss(self, **dynamic_params):
		...
		converted_params = {k: self.param_converter._convert_param(v) for k, v in self.loss_params.items() if k not in exclude_keys}
	...
	def initialize_model(self) -> nn.Module:
		...
		try:
			...
			typed_optional_params = {k: model_class.OPTIONAL_PARAMS[k](v) for k, v in optional_params.items()}
	...
	def create_unique_folder(self):
		...
		"""
		filename = f"{self.model_params.get('model_type', 'UnetVanilla')}__{self.training_time}"
	...
	def load_segmentation_data(self):
		...
		def load_data_stats(data_dir):
			...
			try:
				with open(stats_file, "r") as f:
					raw = json.load(f)
				return {k: [np.array(v[0]), np.array(v[1])] for k, v in raw.items() if isinstance(v, list) and len(v) == 2}
		...
		class_p_scale = {cls: round(min(2.0, 0.5 + (factor / max_factor)), 2) for cls, factor in class_repeat_factor.items()}

		# --- Stratified sampling with forced rare classes ---
		train_idx, val_idx, test_idx = stratified_split_forced(
			self.subfolders, img_data, self.class_to_idx, repeat_factors=class_repeat_factor, val_split=1 - self.val_split, seed=42, max_total=self.num_samples
		)
		...
		test_dataset = TiffDatasetLoader(
			indices=test_idx, img_data=img_data, img_labels=img_labels, data_stats=data_stats, img_res=self.img_res, weights=self.weights, augmentation_params=None, rare_classes=None
		)
		...
		self.dataloaders = {"train": train_loader, "val": val_loader, "test": test_loader, "indices": [train_idx, val_idx, test_idx]}
			...
			for phase in ["train", "val"]:
				...
				with tqdm(total=len(self.dataloaders[phase]), unit="batch") as pbar:
					for inputs, labels in self.dataloaders[phase]:
						...
						pbar.set_postfix(loss=running_loss / total_samples, **{m: running_metrics[m] / total_samples for m in display_metrics})
						pbar.update(1)
				...
				if phase == "val":
					...
					for m, val in epoch_metrics.items():
						if val > best_val_metrics[m]:
							...
							torch.save(self.model.state_dict(), os.path.join(self.save_directory, f"model_best_{m}.pth"))
	...
	def train(self):
		loss_dict, metrics_dict, metrics = self.training_loop(optimizer=self.optimizer, scheduler=self.scheduler)

		# plot and metric saving
		self.logger.save_best_metrics(loss_dict=loss_dict, metrics_dict=metrics_dict)
		self.logger.plot_learning_curves(loss_dict=loss_dict, metrics_dict=metrics_dict)
		...
		if "ConfusionMatrix" in self.metrics:
			self.logger.save_confusion_matrix(conf_metric=metrics[self.metrics.index("ConfusionMatrix")], model=self.model, val_dataloader=self.dataloaders["val"], device=self.device)
