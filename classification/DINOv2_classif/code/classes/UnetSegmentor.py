...
class UnetSegmentor(nn.Module):
	REQUIRED_PARAMS = {"num_classes": int}

	OPTIONAL_PARAMS = {"n_block": int, "channels": int, "k_size": int, "activation": str, "p": float}
	...
	def __init__(self, n_block=4, channels=8, num_classes=3, p=0.5, k_size=3, activation="relu"):
		...
		self.encoder_convs = nn.ModuleList([self._create_encoder_conv_block(channels=self.channels * 2**i) for i in range(0, self.n_block - 1)])
		self.mid_conv = self._create_encoder_conv_block(channels=self.channels * 2 ** (self.n_block - 1))
		self.decoder_deconvs = nn.ModuleList(
			[
				nn.ConvTranspose2d(in_channels=self.channels * 2 ** (i + 1), out_channels=self.channels * 2**i, kernel_size=3, stride=2, padding=1, output_padding=1)
				for i in reversed(range(self.n_block))
			]
		)
		self.decoder_convs = nn.ModuleList([self._create_decoder_conv_block(channels=self.channels * 2**i) for i in reversed(range(self.n_block))])
		self.seg_conv = nn.Conv2d(in_channels=self.channels, out_channels=self.num_classes, kernel_size=3, padding=1)
