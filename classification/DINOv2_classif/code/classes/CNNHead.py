...
class CNNHead(nn.Module):
	def __init__(self, embedding_size, channels=512, num_classes=3, k_size=3, n_features=1, activation="relu"):
		...
		for i in range(self.n_block):
			if self.upscale_fn[i] == "interpolate":
				self.decoder_convs.append(self._create_decoder_conv_block(channels=channels, kernel_size=self.k_size))
			else:
				channels = channels // 4
				self.decoder_convs.append(self._create_decoder_up_conv_block(channels=channels, kernel_size=self.k_size))

		self.seg_conv = nn.Sequential(nn.Conv2d(channels, num_classes, kernel_size=self.k_size, padding=1))
