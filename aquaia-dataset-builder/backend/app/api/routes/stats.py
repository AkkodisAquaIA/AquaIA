...
async def get_stats(db: AsyncSession = Depends(get_db)):
	...
	downloaded = await db.scalar(select(func.count(ImageRecord.id)).where(ImageRecord.local_path.isnot(None))) or 0
	...
	recent_result = await db.execute(select(SearchQuery).order_by(SearchQuery.created_at.desc()).limit(5))
