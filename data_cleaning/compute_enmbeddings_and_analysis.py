...
inter_mean = [
    np.delete(dist_matrix[i], i).mean()
    for i in range(len(classes))
]
...
df = pd.DataFrame({
    "classe": classes,
    "silhouette": [sil_per_class[c] for c in classes],
    "intra": intra_mean,
    "inter": inter_mean,
    "ratio": ratio
})
...
for s in merge_suggestions:
    print(
        f"{s['class_id_1']} ({s['class_name_1']}) "
        f"<-> {s['class_id_2']} ({s['class_name_2']}) "
        f"| dist={s['centroid_distance']:.4f} "
        f"| silhouette={s['mean_silhouette']:.4f} "
